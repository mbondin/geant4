                 ...
                 "housing",pMotherLogical,pMany,pCopyNo),fConstructor(c)
{
  ...
  fHousing_box = new G4Box("housing_box", zz*cm*stretch,zz*cm*stretch,zz*cm*stretch);
  G4Tubs* Cyl1 = new G4Tubs("Cyl1", 0, 0.50*cm*zz, 0.5*cm*zz, 0.0*M_PI*rad,2*M_PI*rad);
 
  G4RotationMatrix* rm4 = new G4RotationMatrix();
  rm4->rotateY(90*deg);
 
  fScint_log = new G4LogicalVolume(Cyl1,G4Material::GetMaterial("G4_POLYSTYRENE"),
                                   "scint_log",0,0,0);
